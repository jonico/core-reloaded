...
public class TFSHandler {
    ...
    public WorkItem createWorkItem(GenericArtifact ga, String collectionName,
        ...
        while (!workItemWasCreated) {
            ...
            try {
                ...
                while (it.hasNext()) {
                    ...
                    if (null != gaFields) {
                        ...
                        if (fieldDef.getReferenceName().equals(
                            ...
                            if (fieldDef.getFieldType().equals(
                                    FieldType.DATETIME)) {
                ...
                if (!(GenericArtifact.VALUE_UNKNOWN.equals(ga
                        .getDepParentTargetArtifactId()))
                        && !(GenericArtifact.VALUE_NONE.equals(ga
                                .getDepParentTargetArtifactId()))) {
    ...
    public void getChangedWorkItems(TFSConnection connection,
        ...
        String finalQuery = get_all_wi_query.replace("?1", workItemType)
                .replace("?2", TFSMetaData.formatDate(lastModifiedDate))
                .replace("?3", projectName);

        WorkItemCollection tasksQueryResults = workItemClient.query(finalQuery,
                null, false);
        ...
        for (WorkItem wit : workItems) {
            ...
            artifactState.setArtifactVersion(Long.parseLong(wit.getFields()
                    .getField(CoreFieldReferenceNames.REVISION)
                    .getOriginalValue().toString()));
    ...
    public void getWorkItem(TFSConnection connection, String collectionName,
        ...
        while (it.hasNext()) {
            ...
            if (field.getReferenceName().equals(
                    CoreFieldReferenceNames.CHANGED_DATE)
                    || field.getReferenceName().equals(
                            CoreFieldReferenceNames.CHANGED_DATE)) {
                    ...
                    if (null != gaFields
                        ...
                        if (fieldDef.getFieldType().equals(FieldType.DATETIME)) {
                            if (DateUtil.isAbsoluteDateInTimezone(
                                    (Date) fieldValue,
                                    ga.getSourceSystemTimezone())) {
                                fieldValue = DateUtil.convertToGMTAbsoluteDate(
                                        (Date) fieldValue,
                                        ga.getSourceSystemTimezone());
                            }
                        }

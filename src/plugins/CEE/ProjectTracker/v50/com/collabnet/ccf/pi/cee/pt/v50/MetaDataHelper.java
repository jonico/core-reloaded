...
public class MetaDataHelper {
    private static final Log                      log                      = LogFactory
                                                                                   .getLog(MetaDataHelper.class);
    private static MetaDataHelper                 instance                 = null;
    private HashMap<String, ArtifactTypeMetadata> artifactTypeMetadataHash = new HashMap<String, ArtifactTypeMetadata>();

    private MetaDataHelper() {
    }

    public ArtifactTypeMetadata getArtifactTypeMetadata(String artifactTypeId) {
        ArtifactTypeMetadata metadata = artifactTypeMetadataHash
                .get(artifactTypeId);
        return metadata;
    }

    public ArtifactTypeMetadata getArtifactTypeMetadata(String repositoryKey,
            String artifactTypeNamespace, String artifactTypeTagName) {
    ...
    public TrackerArtifactType getTrackerArtifactType(String repositoryKey,
            String artifactTypeDisplayName, TrackerWebServicesClient twsclient,
            boolean useCache) throws WSException, RemoteException,
            ServiceException {
        ...
        for (TrackerArtifactType type : tAT) {
            ...
            if (artifactTypeDisplayName.equals(type.getDisplayName())) {
                ArtifactTypeMetadata meta = this.populateArtifactTypeMetadata(
                        repositoryKey, namespace, tagname, twsclient);
    ...
    public ArtifactTypeMetadata populateArtifactTypeMetadata(
            String repositoryKey, String artifactTypeNamespace,
            String artifactTypeTagName, TrackerWebServicesClient twsclient) {
        String artifactTypeFullyQualifiedName = "{" + artifactTypeNamespace
                + "}" + artifactTypeTagName;
        ...
        try {
            ...
            String message = "Web services exception when trying to get the "
                    + "artifact type metadata for "
                    + artifactTypeFullyQualifiedName;
            log.error(message, e);
     ...
     * public TrackerArtifactType getTrackerArtifactType(String repositoryKey,
     * String artifactTypeNamespace, String artifactTypeTagName,
     * TrackerWebServicesClient twsclient){ ArtifactTypeMetadata meta =
     * getInstance().getArtifactTypeMetadata(repositoryKey); if(meta == null){
     * meta = this.populateArtifactTypeMetadata(repositoryKey,
     * artifactTypeNamespace, artifactTypeTagName, twsclient); }
     * TrackerArtifactType newArtifactType = new
     * TrackerArtifactType(null,artifactTypeTagName,artifactTypeNamespace);
     * newArtifactType.populateAttributes(meta); return newArtifactType; }
     */

    public void putArtifactTypeMetadata(String artifactTypeId,

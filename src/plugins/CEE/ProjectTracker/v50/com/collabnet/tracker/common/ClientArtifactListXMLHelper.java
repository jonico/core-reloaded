...
public class ClientArtifactListXMLHelper {
                ...
                if (null != values) {
                    if (values.isEmpty()) {
                        ...
                        for (String attributeValue : values) {
                            clientArtifact.addAttributeValue(
    ...
    private ClientArtifactAttachment getAttachment(Node node) {
        ...
        while (child != null) {
            if (Node.ELEMENT_NODE == child.getNodeType()) {
                ...
                } else if (child.getLocalName().equals(CREATED_ON_TAG)
                        && child.getNamespaceURI().equals(NAMESPACE)) {
                    Node n = child.getFirstChild();
                    createdOn = getNodeValue(n);
                } else if (child.getLocalName().equals(NAME_TAG)
                        && child.getNamespaceURI().equals(NAMESPACE)) {
                    Node n = child.getFirstChild();
                    attachmentName = getNodeValue(n);
                } else if (child.getLocalName().equals(DESCRIPTION_TAG)
                        && child.getNamespaceURI().equals(NAMESPACE)) {
                    Node n = child.getFirstChild();
                    description = getNodeValue(n);
                } else if (child.getLocalName().equals(ID_TAG)
                        && child.getNamespaceURI().equals(NAMESPACE)) {
                    Node n = child.getFirstChild();
                    attachmentId = getNodeValue(n);
                } else if (child.getLocalName().equals(IS_FILE_TAG)
                        && child.getNamespaceURI().equals(NAMESPACE)) {
                    Node n = child.getFirstChild();
                    isFile = getNodeValue(n);
                } else if (child.getLocalName().equals(LOCATION_TAG)
                        && child.getNamespaceURI().equals(NAMESPACE)) {
                    Node n = child.getFirstChild();
                    attachmentLocation = getNodeValue(n);
                } else if (child.getLocalName().equals(MIME_TAG)
                        && child.getNamespaceURI().equals(NAMESPACE)) {
                    Node n = child.getFirstChild();
    ...
    private ClientArtifactComment getComment(Node node) {
        ...
        while (child != null) {
            if (Node.ELEMENT_NODE == child.getNodeType()) {
                ...
                } else if (child.getLocalName().equals(CREATED_BY_TAG)
                        && child.getNamespaceURI().equals(NAMESPACE)) {
                    Node n = child.getFirstChild();
                ...
                } else if (child.getLocalName().equals(TEXT_TAG)
                        && child.getNamespaceURI().equals(NAMESPACE)) {
                    Node n = child.getFirstChild();
    ...
    private void parseErrors(Node errorsNode) {
        ...
        while (errorNode != null) {
            if (errorNode.getLocalName().equals("error")
                    && errorNode.getNamespaceURI().equals(NAMESPACE)) {
                ClientXMLOperationError error = new ClientXMLOperationError(
                        errorNode);
                mErrors.add(error);
            }
    ...
    private void parseResultDocument(Document document) {
        ...
        while (child != null) {
            if (child.getLocalName().equals(PAGE_INFO)
                    && child.getNamespaceURI().equals(NAMESPACE)) {
                setPageInfo(child);
            } else if (child.getLocalName().equals(ERRORS_TAG)
                    && child.getNamespaceURI().equals(NAMESPACE)) {
                parseErrors(child);
            } else {

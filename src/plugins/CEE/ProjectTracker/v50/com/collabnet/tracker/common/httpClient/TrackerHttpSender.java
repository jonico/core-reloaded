...
/**
 * HTTPSender used by axis so that it uses the more full functioned HTTPClient
 * instead of the basic java network functions.
 * 
 * @author Shawn Minto
 * 
 */
public class TrackerHttpSender extends CommonsHTTPSender {
    ...
    public static Credentials getCredentials(AuthenticatedProxy authProxy,
            InetSocketAddress address) {
        return getCredentials(authProxy.getUserName(), authProxy.getPassword(),
                address.getAddress());
    }
    ...
    public static void setupHttpClient(HttpClient client, Proxy proxySettings,
            String repositoryUrl, String user, String password) {
        ...
        if (user != null && password != null) {
            ...
            try {
                ...
                client.getState().setCredentials(authScope,
                        getCredentials(user, password, null));
            }
        }
        ...
        if (isRepositoryHttps(repositoryUrl)) {
            Protocol acceptAllSsl = new Protocol("https",
                    (ProtocolSocketFactory) SslProtocolSocketFactory
                            .getInstance(), getPort(repositoryUrl));

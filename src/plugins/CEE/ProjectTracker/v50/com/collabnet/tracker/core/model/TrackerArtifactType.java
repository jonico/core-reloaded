...
public class TrackerArtifactType implements Serializable {
    ...
    public TrackerArtifactType(String displayName, String tagName,
            String namespace) {
    ...
    /**
     * Tries to see if there is a key that ends with the tagName. ie. we really
     * want "summary" but we'll settle for "c1.0_summary"
     * 
     * @param tagName
     * @return
     */
    ...
    public void populateAttributes(ArtifactTypeMetadata artifactMetaData) {
        ...
        for (Attribute attribute : attributes) {
            ...
            TrackerAttribute trackerAttribute = new TrackerAttribute(
                    attributeDisplayName, attributeTagName, attributeNamespace,
        ...
        for (TrackerReportElement element : TrackerReportElement.values()) {
            if (element.isGlobal()) {
                TrackerAttribute trackerAttribute = new TrackerAttribute(
                        element.toString(), element.getTagName(),
        ...
        addAttribute(new TrackerAttribute(
                TrackerReportElement.NEW_COMMENT.toString(),
                TrackerReportElement.NEW_COMMENT.getTagName(),
                TrackerReportElement.NEW_COMMENT.getNamespace(), true, true));

        addAttribute(new TrackerAttribute(
                TrackerReportElement.REASON_FOR_CHANGE.toString(),
                TrackerReportElement.REASON_FOR_CHANGE.getTagName(),
                TrackerReportElement.REASON_FOR_CHANGE.getNamespace(), true,
                true));
    }
    ...
    private void setReadOnlyAndHidden(TrackerAttribute trackerAttribute) {
        ...
        if (key.equals(TrackerReportElement.ID.getKeyString())) {
            ...
            trackerAttribute.setHidden(true);
                ...
                .endsWith("}" + TrackerReportElement.SUMMARY.getTagName())) {
            trackerAttribute.setReadOnly(false);
            trackerAttribute.setHidden(true);
        } else if (key.endsWith("}"
                + TrackerReportElement.DESCRIPTION.getTagName())) {
            trackerAttribute.setReadOnly(false);
            trackerAttribute.setHidden(true);
        } else if (key.endsWith("}"
                + TrackerReportElement.ASSIGNED_TO.getTagName())) {
            trackerAttribute.setReadOnly(false);
            trackerAttribute.setHidden(false);
        } else if (key.endsWith("}"
                + TrackerReportElement.CARBON_COPY.getTagName())) {
            trackerAttribute.setReadOnly(false);
            trackerAttribute.setHidden(false);

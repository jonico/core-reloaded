...
public class QCConfigHelper {
    ...
    public Set<String> getFieldsNamesOfType(IConnection connection,
        ...
        String sql = String
                .format("SELECT SF_COLUMN_NAME FROM SYSTEM_FIELD WHERE SF_TABLE_NAME='%s' AND SF_COLUMN_TYPE='%s' AND SF_USER_LABEL IS NOT NULL",
                        trackerType, fieldValueType);
        IRecordSet rs = null;
        try {
    ...
    public static GenericArtifact getSchemaFieldsForDefect(IConnection qcc) {
        ...
        try {
            ...
            for (int cnt = 0; cnt < rc; cnt++, rs.next()) {
                ...
                field.setMaxOccursValue(isMultiSelectField ? GenericArtifactField.UNBOUNDED
                        : "1");
    ...
    public static GenericArtifact getSchemaFieldsForRequirement(
            IConnection qcc, String technicalReleaseTypeId) {
        ...
        try {
            ...
            for (int cnt = 0; cnt < rc; cnt++, rs.next()) {
                ...
                if (fieldValueType
                    ...
                    isMultiSelectField = "ReleaseCycleMultiTreeCombo"
                            .equals(editStyle)
    ...
    private static GenericArtifactField.FieldValueTypeValue convertQCDataTypeToGADatatype(
        ...
        log.debug("Unknown QC data type " + dataType + " of field "
                + columnName + " defaulting to STRING");
        return GenericArtifactField.FieldValueTypeValue.STRING;
    }
        ...
        if (null == s_JoinedFields) {
            ...
            try {
                ...
                for (int cnt = 0; cnt < rc; cnt++, rs.next()) {
                    ...
                    String tableName = rs
                            .getFieldValueAsString(sfJoinParentTable);
                    String joinTableName = rs
                            .getFieldValueAsString(sfJoinTable);
                    String joinRowKey = rs.getFieldValueAsString(sfJoinRowKey);
                    String joinRowDisplayName = rs
                            .getFieldValueAsString(sfJoinRowDisplayName);
    ...
    static List<String> getJoinedFieldDisplayNames(IConnection qcc,
            boolean isDefect, String fieldName) {
    ...
    static boolean isJoinedField(IConnection qcc, boolean isDefect,
            String fieldName) {
     ...
     */
    static String mapJoinedFieldDisplayToId(IConnection qcc, boolean isDefect,
        ...
        if (null == id) {
            String msg = new StringBuilder(
                    ...
                    .append(": ").append(displayValue).toString();
            //log.error(msg);
     ...
     * attachmentName) { GenericArtifactAttachment attachment; List<String>
     * attachmentIdAndType = QCDefectHandler.getFromTable(qcc, entityId,
     * attachmentName); if(attachmentIdAndType!=null) { String attachmentId =
     ...
     * .AttachmentActionValue.CREATE); } return genericArtifact; } public static
     * GenericArtifact getCompleteSchemaAttachments(IConnection qcc, String
     * actionId, String entityId, List<String> attachmentNames) {
     * GenericArtifact genericArtifact = new GenericArtifact();
     * if(attachmentNames!=null) { for(int cnt=0; cnt < attachmentNames.size();
     * cnt++) { GenericArtifactAttachment attachment; List<String>
     * attachmentIdAndType = QCDefectHandler.getFromTable(qcc, entityId,
     * attachmentNames.get(cnt)); if(attachmentIdAndType!=null) { String
     * attachmentId = attachmentIdAndType.get(0); // CR_REF_ID String
     * attachmentContentType = attachmentIdAndType.get(1); // CR_REF_TYPE String
     * attachmentDescription = attachmentIdAndType.get(2); // CR_DESCRIPTION
     * if(attachmentContentType.equals("File")) { attachment =
     ...
     * .AttachmentContentTypeValue.LINK); }
     * attachment.setAttachmentAction(GenericArtifactAttachment
     * .AttachmentActionValue.CREATE); } } } return genericArtifact; }
     */

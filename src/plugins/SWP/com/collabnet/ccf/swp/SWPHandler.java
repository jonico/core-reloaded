...
public class SWPHandler {
            ...
            for (GenericArtifactField comment : comments) {
                if (comment.getFieldValue() != null) {
                    ...
                    if (!commentValue.isEmpty()) {
                        endpoint.createCommentForBacklogItem(pbi.getId(),
                                commentValue);
                    }
                }
            }
        ...
        if (pointPerson != null) {
            ...
            if (anticipatedPointPerson != null
                ...
                try {
                    ...
                    log.warn("Could not retrieve user object for anticipated point person "
        ...
        if (comments != null) {
            for (GenericArtifactField comment : comments) {
                if (comment.getFieldValue() != null) {
                    ...
                    // does not accept empty values
                    if (!commentValue.isEmpty()) {
                        endpoint.createCommentForTask(task.getId(),
                                commentValue);
                    }
                }
            }
    ...
    public void getChangedPBIs(ScrumWorksAPIService endpoint,
        ...
        while (it.hasPrevious()) {
            ...
            if (!deletedArtifacts.isEmpty()) {
                ...
                int processedMinorVersion = (numberOfChangedOrAddedArtifacts + numberOfDeletedArtifacts) * 2;
                ListIterator<Long> it2 = deletedArtifacts
                        .listIterator(numberOfDeletedArtifacts);
                while (it2.hasPrevious()) {
                    Long deletedArtifactId = it2.previous();
                    /*
                     * Check whether we already transported this artifact or if
                     * the current revision number is greater than the major
                     * version the change has to transported. If this condition
                     * is not true, the major revision is equal to the current
                     * revision number (since earlier versions are not returned
                     * by the API) and we have to use the minor version to
                     * determine what has been already shipped
                     */
    ...
    public void getChangedProducts(ScrumWorksAPIService endpoint,
        ...
        while (it.hasPrevious()) {
            ...
            if (!changedOrAddedProducts.isEmpty()) {
                // now we have to sort the changed/created artifacts
    ...
    public void getChangedReleases(ScrumWorksAPIService endpoint,
        ...
        while (it.hasPrevious()) {
            ...
            if (!changedOrAddedReleases.isEmpty()) {
                // now we have to sort the changed/created artifacts
    ...
    public void getChangedTasks(ScrumWorksAPIService endpoint,
         ...
         * .listIterator(numberOfChangedOrAddedArtifacts); while
         * (it2.hasPrevious()) { Task artifact = it2.previous(); // now prepend
         * item to the list ArtifactState artifactState = new ArtifactState();
         * artifactState.setArtifactId(artifact.getId().toString());
         * XMLGregorianCalendar xmlTimestamp = processedRevisionInfo
         * .getTimeStamp(); Date artifactLastModifiedDate = new Date(0); if
         * (xmlTimestamp != null) { artifactLastModifiedDate = xmlTimestamp
        ...
        while (it.hasPrevious()) {
            ...
            if (!changedOrAddedTasks.isEmpty()) {
                // now we have to sort the changed/created artifacts
                ...
                if (majorVersion < processedRevisionNumber
                    ...
                    if (!alreadyProcessedArtifacts.contains(artifact.getId())) {
                        ...
                        try {
                            ...
                            log.debug(
                                    "While retrieving parent pbi "
                                            + artifact.getBacklogItemId()
                                            + " of task " + artifact.getId()
     ...
     * @throws ScrumWorksException
     */
    public RevisionInfo getCurrentRevision(ScrumWorksAPIService endpoint)
            throws ScrumWorksException {
    ...
    public void retrieveMetaData(ScrumWorksAPIService endpoint, String id,
        ...
        for (Team team : teams) {
            ...
            for (Sprint sprint : sprints) {
                sprintsPresent = true;
                addMetaDataField(ga, MetaDataFields.teamSprint,
                        SWPMetaData.getTeamSprintStringRepresentation(sprint,
                                team, ga.getSourceSystemTimezone()));
                addMetaDataField(ga, MetaDataFields.sprintTeam,
                        SWPMetaData.getSprintTeamStringRepresentation(sprint,
                                team, ga.getSourceSystemTimezone()));
            }
        }
        if (!teamsPresent) {
    ...
    public void retrievePBI(ScrumWorksAPIService endpoint, String id,
        ...
        if (pbi != null) {
            ...
            GenericArtifactField completedDateField = addPBIField(ga,
                    PBIFields.completedDate, pbi.getCompletedDate());
            SWPMetaData.setDateFieldValue(pbi.getCompletedDate(),
                    ga.getSourceSystemTimezone(), completedDateField);
            ...
            if (sprintId == null) {
                ...
                GenericArtifactField sprintStartField = addPBIField(ga,
                        PBIFields.sprintStart, sprint.getStartDate());
                SWPMetaData.setDateFieldValue(sprint.getStartDate(),
                        ga.getSourceSystemTimezone(), sprintStartField);
                GenericArtifactField sprintEndField = addPBIField(ga,
                        PBIFields.sprintEnd, sprint.getEndDate());
                SWPMetaData.setDateFieldValue(sprint.getEndDate(),
                        ga.getSourceSystemTimezone(), sprintEndField);
                // retrieve team name
    ...
    public void retrieveProduct(ScrumWorksAPIService endpoint, String id,
        ...
        if (product != null) {
            ...
            addProductField(genericArtifact, ProductFields.keyPrefix,
                    product.getKeyPrefix());
            addProductField(genericArtifact, ProductFields.name,
                    product.getName());
            addProductField(genericArtifact, ProductFields.trackTimeSpent,
                    product.isTrackTimeSpent());
        } else {
            genericArtifact.setArtifactAction(ArtifactActionValue.DELETE);
        }
    ...
    public void retrieveRelease(ScrumWorksAPIService endpoint, String id,
        ...
        if (release != null) {
            ...
            addProductReleaseField(ga, ReleaseFields.archived,
                    release.isArchived());
            addProductReleaseField(ga, ReleaseFields.description,
            ...
            GenericArtifactField releaseDateField = addProductReleaseField(ga,
                    ReleaseFields.releaseDate, release.getEndDate());
            SWPMetaData.setDateFieldValue(release.getEndDate(),
                    ga.getSourceSystemTimezone(), releaseDateField);

            GenericArtifactField startDateField = addProductReleaseField(ga,
                    ReleaseFields.startDate, release.getStartDate());
            SWPMetaData.setDateFieldValue(release.getStartDate(),
    ...
    public void retrieveTask(ScrumWorksAPIService endpoint, String id,
        ...
        if (task != null) {
            ...
            addTaskField(genericArtifact, TaskFields.description,
                    task.getDescription());
            addTaskField(genericArtifact, TaskFields.estimatedHours,
                    task.getCurrentEstimate());
            addTaskField(genericArtifact, TaskFields.originalEstimate,
                    task.getOriginalEstimate());
            addTaskField(genericArtifact, TaskFields.backlogItemId,
                    task.getBacklogItemId());
            addTaskField(genericArtifact, TaskFields.pointPerson,
                    task.getPointPerson());
            addTaskField(genericArtifact, TaskFields.rank, task.getRank());
            addTaskField(genericArtifact, TaskFields.status, task.getStatus());
            // addTaskField(genericArtifact, TaskFields.taskBoardStatusRank,
            // task
            // .get);
            ...
            if (!anticipatedThemeNames.isEmpty()) {
                ...
                if (availableThemes == null || availableThemes.size() == 0) {
                    ...
                    if (!anticipatedThemeNames.isEmpty()) {
                        ...
                        log.warn("Since this could be due to a theme or program rename operation, we will only apply additional themes but not remove already set ones ...");
                    } else {
    ...
    public Task updateTask(ScrumWorksAPIService endpoint,
        ...
        if (parent == null
                ...
                || !SWPMetaData.retrieveSWPTypeFromRepositoryId(
                        parentRepository).equals(SWPType.PBI)) {
            // do nothing
            log.warn("It looks as if somebody has deleted the parent child relationship between task "
                    + task.getId()
                    + " and PBI "
                    + task.getBacklogItemId()
    ...
    private void addMetaDataField(GenericArtifact genericArtifact,
        ...
        GenericArtifactField gaField = genericArtifact.addNewField(
                field.getFieldName(), "mandatoryField");
     ...
     */
    private GenericArtifactField addPBIField(GenericArtifact genericArtifact,
            PBIFields field, Object value) {
        // all fields are from field type "mandatoryField" since SWP has a
        // static field model
        GenericArtifactField gaField = genericArtifact.addNewField(
                field.getFieldName(), "mandatoryField");
     ...
     */
    private GenericArtifactField addProductReleaseField(
            GenericArtifact genericArtifact, ReleaseFields field, Object value) {
        // all fields are from field type "mandatoryField" since SWP has a
        // static field model
        GenericArtifactField gaField = genericArtifact.addNewField(
                field.getFieldName(), "mandatoryField");
    ...
    private Long getProductId(String productName, ScrumWorksAPIService endpoint)
        ...
        if (productId == null) {
            ...
            if (indexLeftBracket == -1 || indexRightBracket == -1) {
                ...
                productId = Long.parseLong(productName.substring(
                        indexLeftBracket + 1, indexRightBracket));
            }

...
public class GenericArtifactHelper {
    ...
    public static GenericArtifact createGenericArtifactJavaObject(
        ...
        if (ArtifactTypeValue.DEPENDENCY == artifactType
                ...
                || ArtifactTypeValue.PLAINARTIFACT == artifactType) {
        ...
        for (Element field : fields) {
            ...
            if (includesFieldMetaData
                ...
                String alternativeFieldName = getAttributeValueWithoutException(
                        field, ALTERNATIVE_FIELD_NAME);
                if (null != alternativeFieldName) {
                    genericArtifactField
                            .setAlternativeFieldName(alternativeFieldName);
                }
            }
    ...
    public static Document createGenericArtifactXMLDocument(
        ...
        switch (genericArtifact.getArtifactMode()) {
            case CHANGEDFIELDSONLY: {
                addAttribute(root, ARTIFACT_MODE,
                        ARTIFACT_MODE_CHANGED_FIELDS_ONLY);
                break;
            }
            case COMPLETE: {
        ...
        addAttribute(root, SOURCE_ARTIFACT_VERSION,
                genericArtifact.getSourceArtifactVersion());
        addAttribute(root, TARGET_ARTIFACT_VERSION,
                genericArtifact.getTargetArtifactVersion());
        addAttribute(root, CONFLICT_RESOLUTION_PRIORITY,
                genericArtifact.getConflictResolutionPriority());

        // only create optional attributes if necessary
        if (ArtifactTypeValue.DEPENDENCY == artifactType
                || ArtifactTypeValue.ATTACHMENT == artifactType
                || ArtifactTypeValue.PLAINARTIFACT == artifactType) {
            addAttribute(root, DEP_PARENT_SOURCE_ARTIFACT_ID,
                    genericArtifact.getDepParentSourceArtifactId());
            addAttribute(root, DEP_PARENT_SOURCE_REPOSITORY_ID,
                    genericArtifact.getDepParentSourceRepositoryId());
            addAttribute(root, DEP_PARENT_SOURCE_REPOSITORY_KIND,
                    genericArtifact.getDepParentSourceRepositoryKind());
            addAttribute(root, DEP_PARENT_TARGET_ARTIFACT_ID,
                    genericArtifact.getDepParentTargetArtifactId());
            addAttribute(root, DEP_PARENT_TARGET_REPOSITORY_ID,
                    genericArtifact.getDepParentTargetRepositoryId());
        ...
        // dependencies have even more optional attributes
        if (ArtifactTypeValue.DEPENDENCY == artifactType) {
            addAttribute(root, DEP_CHILD_SOURCE_ARTIFACT_ID,
                    genericArtifact.getDepChildSourceArtifactId());
            addAttribute(root, DEP_CHILD_SOURCE_REPOSITORY_ID,
                    genericArtifact.getDepChildSourceRepositoryId());
            ...
            addAttribute(root, DEP_CHILD_TARGET_ARTIFACT_ID,
                    genericArtifact.getDepChildTargetArtifactId());
            addAttribute(root, DEP_CHILD_TARGET_REPOSITORY_ID,
        ...
        addAttribute(root, SOURCE_ARTIFACT_ID,
                genericArtifact.getSourceArtifactId());
        addAttribute(root, SOURCE_REPOSITORY_ID,
                genericArtifact.getSourceRepositoryId());
        addAttribute(root, SOURCE_REPOSITORY_KIND,
                genericArtifact.getSourceRepositoryKind());
        addAttribute(root, SOURCE_SYSTEM_ID,
                genericArtifact.getSourceSystemId());
        addAttribute(root, SOURCE_SYSTEM_KIND,
                genericArtifact.getSourceSystemKind());
        addAttribute(root, TARGET_ARTIFACT_ID,
                genericArtifact.getTargetArtifactId());
        addAttribute(root, TARGET_REPOSITORY_ID,
                genericArtifact.getTargetRepositoryId());
        addAttribute(root, TARGET_REPOSITORY_KIND,
                genericArtifact.getTargetRepositoryKind());
        addAttribute(root, TARGET_SYSTEM_ID,
                genericArtifact.getTargetSystemId());
        addAttribute(root, TARGET_SYSTEM_KIND,
                genericArtifact.getTargetSystemKind());
        addAttribute(root, SOURCE_SYSTEM_TIMEZONE,
                genericArtifact.getSourceSystemTimezone());
        addAttribute(root, TARGET_SYSTEM_TIMEZONE,
                genericArtifact.getTargetSystemTimezone());
        ...
        if (null != genericArtifact.getAllGenericArtifactFields()) {
            ...
            for (GenericArtifactField genericArtifactField : genericArtifact
                ...
                addAttribute(field, FIELD_NAME,
                        genericArtifactField.getFieldName());
                addAttribute(field, FIELD_TYPE,
                        genericArtifactField.getFieldType());
                ...
                if (genericArtifact.getIncludesFieldMetaData().equals(
                        GenericArtifact.IncludesFieldMetaDataValue.TRUE)) {
                    addAttribute(field, MIN_OCCURS,
                            genericArtifactField.getMinOccursValue());
                    addAttribute(field, MAX_OCCURS,
                            genericArtifactField.getMaxOccursValue());
                    addAttribute(field, NULL_VALUE_SUPPORTED,
                        ...
                        && null != genericArtifactField.getAlternativeFieldName()) {
    ...
    private static void setFieldValue(Element field, Object fieldValue,
        ...
        if (null == fieldValue) {
            ...
            if (fieldValue instanceof Date)
                ...
                synchronized (df) {
                    setValue(field,
                            df.format(((Calendar) fieldValue).getTime()), false);
                }
            else if (fieldValue instanceof XMLGregorianCalendar)
                synchronized (df) {
                    setValue(field,
                            df.format(((XMLGregorianCalendar) fieldValue)

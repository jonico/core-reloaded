 ...
 * 
 */
public class MappingDBUpdater2 extends LifecycleComponent implements IDataProcessor {
    ...
    private static final Log    log                                  = LogFactory
                                                                             .getLog(MappingDBUpdater2.class);
    ...
    public Object[] process(Object data) {
        ...
        if (data instanceof Document) {
            ...
            try {
                 ...
                 * system kind and target system kind top level attributes.
                 * These top level attributes have not been used in CCF 1.x but
                 * have already been mapped so that CCF 1.x mappings will
                 * continue to work with CCF 2.x without any modifications.
                 */
                ...
                String repositoryMappingId = XPathUtils.getAttributeValue(
                ...
                if (artifactAction
                    ...
                    if (!replayedArtifact
                        ...
                        updateSynchronizationStatusTable(data,
                                ...
                                sourceTime, repositoryMappingDirectionId);
                    }
                ...
                if (!replayedArtifact
                    ...
                    updateSynchronizationStatusTable(data, sourceArtifactId,
                            sourceArtifactVersion, sourceTime,
    ...
    public void validate(List exceptions) {
        ...
        if (null == sourceSystemIsTeamForge) {
            log.error("sourceSystemIsTeamForge-property not set");
            exceptions.add(new ValidationException(
                    "sourceSystemIsTeamForge-property not set", this));
        }
    }
    ...
    private void createIdentityMapping(String sourceArtifactId,
        ...
        /*
         * Starting from CCF 2.x we only have one identity mapping row for both
         * directions Consequently, we have to know whether this is a forward or
         * reverse process to properly create the entry
         */
        ...
        inputParameters.add(2, DESCRIPTION,
                "This identity mapping has been inserted by CCF Core");

        inputParameters.add(3, SOURCE_ARTIFACT_ID, b ? sourceArtifactId
                : targetArtifactId);
        inputParameters.add(4, TARGET_ARTIFACT_ID, b ? targetArtifactId
                : sourceArtifactId);
        inputParameters.add(5, SOURCE_LAST_MODIFICATION_TIME, b ? sourceTime
                : targetTime);
        inputParameters.add(6, TARGET_LAST_MODIFICATION_TIME, b ? targetTime
                : sourceTime);
        inputParameters.add(7, SOURCE_ARTIFACT_VERSION,
                b ? sourceArtifactVersion : targetArtifactVersion);
        inputParameters.add(8, TARGET_ARTIFACT_VERSION,
                b ? targetArtifactVersion : sourceArtifactVersion);
        ...
        inputParameters
                .add(12, DEP_CHILD_SOURCE_REPOSITORY_KIND,
                        b ? depChildSourceRepositoryKind
        ...
        inputParameters
                .add(15, DEP_CHILD_TARGET_REPOSITORY_KIND,
                        b ? depChildTargetRepositoryKind
        ...
        inputParameters.add(18, DEP_PARENT_SOURCE_REPOSITORY_KIND,
                b ? depParentSourceRepositoryKind
    ...
    private void createMapping(Element element, String sourceArtifactId,
        ...
        if (null == targetArtifactIdFromTable) {
            ...
            this.updateIdentityMapping(sourceArtifactId, sourceTime,
                    targetTime, sourceArtifactVersion, targetArtifactVersion,
                    artifactType, repositoryMappingId);
    ...
    private void updateSynchronizationStatusTable(Object data,
        ...
        inputParameters.add(3, REPOSITORY_MAPPING_DIRECTION,
                repositoryMappingDirectionId);

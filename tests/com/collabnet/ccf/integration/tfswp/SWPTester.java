...
public class SWPTester {
    ...
    /**
     * Returns the backlog items after the title of a backlog item has been
     ...
     *             if an error occurs from ScrumWorks
     */
    public BacklogItem waitForBacklogItemToUpdate(final String title)
            throws InterruptedException, ScrumWorksException {
        List<BacklogItem> backlogItems = new ArrayList<BacklogItem>();
        for (int i = 0; i < ccfMaxWaitTime; i += ccfRetryInterval) {
            backlogItems = getSWPEndpoint().getBacklogItemsInProduct(
                    getProduct().getId(), false);
        ...
        throw new RuntimeException("Backlog items with the given title: "
                + title + " were not found within the given time: "
                + ccfMaxWaitTime);
    }

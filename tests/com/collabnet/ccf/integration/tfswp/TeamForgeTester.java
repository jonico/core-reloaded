...
public class TeamForgeTester {
     ...
     * 
     * @param parentId
     *            the id for the parent artifact
     * @param childId
     *            the id for the child artifact
     * @param message
     *            text describing the type of relationship
     ...
     */
    public void createArtifactDependency(final String parentId,
            final String childId, final String message) throws RemoteException,
    ...
    public ArtifactDO createBacklogItem(final String title,
            ...
            final FieldValues flexFields, Integer effort)
            throws RemoteException {
     ...
     *            the task title
     * @param description
     *            the task description, null for empty <blank> description
     * @param status
     *            the status, null for Not Started
     * @param assignedUsername
     *            the assigned user for the task, null for the TeamForge test
     *            user
     * @param remainingEffort
     *            the remaining task hours
     * @param originalEstimate
     *            the original task estimate
     * @return the task artifact
     * @throws RemoteException
     *             if TeamForge can not be reached
     */
    public ArtifactDO createTask(String title, String description,
        ...
        final String taskDescription = description == null ? "<blank>"
                : description;
        final String taskStatus = status == null ? TaskStatus.NOT_STARTED
                .getStatus() : status;
        final String taskUser = assignedUsername == null ? getUserName()
                : assignedUsername;
    ...
    /**
     * Creates a backlog item titled TestTitle and a child task with the given
     * values.
     * 
     * @param title
     *            the task title
    ...
    public ArtifactDO createTaskAndPBI(String title, String description,
        ...
        createArtifactDependency(pbi.getId(), task.getId(),
                "Parent-child relationship created by unit test");
        return task;
    }
    ...
    public void deleteAllPBIsInTF() throws RemoteException {
        ArtifactRow[] tfRows = connection.getTrackerClient()
                .getArtifactList(getPbiTracker(), null).getDataRows();
    ...
    public void deleteAllTasksInTF() throws RemoteException {
        ArtifactRow[] tfRows = connection.getTrackerClient()
                .getArtifactList(getTaskTracker(), null).getDataRows();
        for (ArtifactRow artifactRow : tfRows) {
            connection.getTrackerClient().deleteArtifact(artifactRow.getId());
        }
    }
    ...
    public ArtifactDO updateBacklogItem(final String backlogItemId,
            ...
            String status, final FieldValues flexFields, final Integer effort)
            throws RemoteException, PlanningFolderRuleViolationException {
    ...
    public ArtifactRow[] waitForBacklogItemsToAppear(final int numberOfPbis)
        ...
        return waitForArtifactToAppear(numberOfPbis, getPbiTracker(),
                "backlog item");
    }
    ...
    public ArtifactRow[] waitForBacklogItemsToDisappear(final int numberOfPbis)
            throws RemoteException, InterruptedException {
        return waitForArtifactToDisappear(numberOfPbis, getPbiTracker(),
                "backlog item");
    }

    /**
     * Returns the updated backlog item from TeamForge after the backlog item
     * has been updated.
     * 
     * @param title
     *            the updated title
     * @param numberOfBacklogItems
     *            the number of backlog items in TeamForge
     * @return the updated backlog item
     * @throws Exception
     *             if an error occurs
     */
    public ArtifactRow waitForBacklogItemToUpdate(final String title,
            final int numberOfBacklogItems) throws Exception {
        ...
        throw new RuntimeException("Backlog item with the given title: "
                + title + " was not updated in the time: " + ccfMaxWaitTime);
    }
    ...
    public PlanningFolderRow[] waitForPlanningFoldersToAppear(String parent,
        ...
        for (int i = 0; i < ccfMaxWaitTime; i += ccfRetryInterval) {
            pfRows = connection.getPlanningClient()
                    .getPlanningFolderList(parent, recursive).getDataRows();
    ...
    public ArtifactRow[] waitForTasksToDisappear(final int numberOfTasks)
            throws RemoteException, InterruptedException {
        return waitForArtifactToDisappear(numberOfTasks, getTaskTracker(),
                "task");
    }
     ...
     * @throws Exception
     */
    public ArtifactRow waitForTaskToUpdate(final String taskTitle,
            final int numberOfTasks) throws Exception {
        for (int i = 0; i < ccfMaxWaitTime; i += ccfRetryInterval) {
    ...
    /**
     * Waits until only the requested number of requested artifacts is still
     * present.
     * 
     * @param numberOfArtifacts
     ...
     *             if the thread can not sleep
     */
    private ArtifactRow[] waitForArtifactToDisappear(
            final int numberOfArtifacts, String artifactTypeId,
            String entityType) throws RemoteException, InterruptedException {
        assert artifactTypeId != null : "null artifact type";
        assert entityType != null : "null entity type";
        ...
        throw new RemoteException(numberOfArtifacts + " " + entityType
                + "(s) were not reached within the given time: "
                + ccfMaxWaitTime);
    }

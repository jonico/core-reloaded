...
public class SFEEReaderTest {
    ...
    public void dummyTestAttachmentsList()
        ...
        Connection connection = sfeeReader.connect(systemId, systemKind,
                repositoryId, repositoryKind, connectionInfo, credentialInfo);
        ISourceForgeSoap soapApp = connection.getSfSoap();
        try {

            AttachmentSoapList list = soapApp.listAttachments(
                    connection.getSessionId(), "artf1179");
    ...
    public void dummyTestCommentsList() throws MaxConnectionsReachedException,
            ConnectionException {
        Connection connection = sfeeReader.connect(systemId, systemKind,
                repositoryId, repositoryKind, connectionInfo, credentialInfo);
        ISourceForgeSoap soapApp = connection.getSfSoap();
        try {
            ...
            CommentSoapList list = soapApp.getCommentList(
                    connection.getSessionId(), "artf1054");
    ...
    public void dummyTestWriteAttachment()
        ...
        Connection connection = sfeeReader.connect(systemId, systemKind,
                repositoryId, repositoryKind, connectionInfo, credentialInfo);
        ISimpleFileStorageAppSoap fileStorageApp = (ISimpleFileStorageAppSoap) ClientSoapStubFactory
                .getSoapStub(ISimpleFileStorageAppSoap.class, serverUrl);
        ITrackerAppSoap mTrackerApp = (ITrackerAppSoap) ClientSoapStubFactory
                .getSoapStub(ITrackerAppSoap.class, serverUrl);
        byte[] b = "Hello this is my simple attachment".getBytes();
        try {
            String fileDescriptor = fileStorageApp.startFileUpload(connection
                    .getSessionId());
            System.out.println("File descriptor " + fileDescriptor);
            fileStorageApp.write(connection.getSessionId(), fileDescriptor, b);
            fileStorageApp.endFileUpload(connection.getSessionId(),
                    fileDescriptor);

            ArtifactSoapDO soapDo = mTrackerApp.getArtifactData(
                    connection.getSessionId(), "artf1179");
            mTrackerApp.setArtifactData(connection.getSessionId(), soapDo,
                    "Adding file via Java", "myFile.txt", "text/plain",
                    fileDescriptor);
        } catch (RemoteException e) {
            e.printStackTrace();
            fail();
    ...
    public void testArtfactRetreival() throws RemoteException,
        ...
        ITrackerAppSoap trackerApp = (ITrackerAppSoap) ClientSoapStubFactory
                .getSoapStub(ITrackerAppSoap.class, serverUrl);
        ...
        ArtifactDetailSoapRow[] rows = trackerApp.getArtifactDetailList(
                connection.getSessionId(), repositoryId, selectedColumns,
                filter, sortKeys, 0, -1, false, false).getDataRows();
        for (ArtifactDetailSoapRow row : rows) {
            ...
            ArtifactDetailSoapRow[] rows1 = trackerApp.getArtifactDetailList(
                    connection.getSessionId(), repositoryId, selectedColumns1,
